...
import TireSpecs from '@/components/Product/TireSpecs';
import TireVideoReviews from '@/components/Product/TireVideoReviews';
import TireUserReviews from '@/components/Product/TireUserReviews';

export default function TireProductPage({ params }: { params: { id: string } }) {
  // Mock data for the specific tire
  const tire = {
    manufacturer: 'Michelin',
    model: 'Pilot Sport 4 S',
    category: 'Ultra High Performance Summer',
    flag: '🇫🇷',
    specs: '245/40 R18 97Y XL',
    rating: 4.9,
    reviews: 142,
    minPrice: 22450,
    offersCount: 8,
    photo: '/tires/michelin.png',
    description: 'Легендарная спортивная шина для мощных автомобилей. Обеспечивает невероятное сцепление на сухом и мокром асфальте, точные реакции на руль и короткий тормозной путь благодаря технологии Bi-Compound.',
  };
    ...
    <div className="min-h-screen bg-slate-50 dark:bg-[#020817] pt-28 pb-20 relative overflow-hidden">
      ...
      <div className="max-w-[1400px] mx-auto px-4 md:px-8 relative z-10">
        ...
        <div className="flex items-center gap-2 text-[11px] font-bold tracking-wider uppercase text-slate-500 mb-8">
          ...
          <a href="/tires" className="hover:text-cyan-500 transition-colors">Шины</a>
          <ChevronRight className="w-3 h-3" />
          <span className="text-slate-900 dark:text-white">{tire.manufacturer}</span>
        </div>
          ...
          <div className="lg:col-span-8 flex flex-col gap-8">
            <GlowingCard className="w-full flex" contentClassName="p-0 flex flex-col md:flex-row h-full items-stretch">
              ...
              <div className="relative w-full md:w-1/2 p-8 flex items-center justify-center bg-slate-100 dark:bg-slate-800/40 border-r border-transparent mdborder-slate-100 dark:md:border-slate-800/60 rounded-t-[1.3rem] md:rounded-l-[1.3rem] md:rounded-tr-none">
                ...
                <img 
                  src={tire.photo} 
                  alt={`${tire.manufacturer} ${tire.model}`} 
                  className="relative z-10 w-full max-w-[360px] object-contain drop-shadow-2xl"
                />
              </div>
              ...
              <div className="flex-1 p-8 flex flex-col justify-center">
                <div className="flex items-center gap-3 mb-2">
                  <span className="text-lg" title="Страна бренда">{tire.flag}</span>
                ...
                <h1 className="text-4xl md:text-5xl font-black text-slate-900 dark:text-white leading-tight tracking-tight mb-4">
                  <span className="block text-2xl md:text-3xl font-bold text-slate-500 mb-1">{tire.manufacturer}</span>
                  {tire.model}
                </h1>
                ...
                <div className="flex items-center gap-6 mb-8">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-xl font-black text-slate-900 dark:text-white">{tire.rating}</span>
                ...
                <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mt-auto">
                  {[
                    { label: 'Сцепление', val: 95 },
                    { label: 'Шум', val: 80, invert: true },
                    { label: 'Износ', val: 88 },
                    { label: 'Топливо', val: 75 },
                  ].map((metric, i) => (
            ...
            <div className="sticky top-32">
              <GlowingCard className="w-full flex flex-col" contentClassName="p-6 flex flex-col h-full">
                ...
                <div className="flex flex-col mb-8 text-center bg-slate-50 dark:bg-slate-800/50 p-6 rounded-2xl border border-slate-100 dark:border-slate-700/50">
                  ...
                  <div className="flex items-baseline justify-center gap-2">
                    <span className="text-5xl font-black font-mono tracking-tighter text-slate-900 dark:text-white">
                      {tire.minPrice.toLocaleString('ru-RU')}
                    </span>
                ...
                <div className="flex flex-col gap-3 mb-8">
                  {[
                    { name: 'Koleso.ru', price: 22450, delivery: 'Завтра', tag: 'Лучшая цена' },
                    { name: 'ШинСервис', price: 23100, delivery: 'Сегодня', tag: 'Быстрее' },
                    { name: 'Мосавтошина', price: 23000, delivery: 'Чт, 24 марта', tag: null },
                  ].map((offer, idx) => (
